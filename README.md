...
$$r = \frac{\sum\sb{i\sb{1}}\sp{n} (X\sb{i} - \bar{X})(Y\sb{i} - \bar{Y})}{\sqrt{\sum\sb{i\sb{1}}\sp{n} (X\sb{i} - \bar{X})\sp{2} \sum\sb{i\sb{1}}\sp{n} (Y\sb{i} - \bar{Y})\sp{2}}}$$
...
$$\text{rentals} = \beta\sb{0} + \beta\sb{1} \text{temp} + \beta\sb{2} \text{precip} + \beta\sb{3} \text{pm10} + \sum\sb{k=1}\sp{2} \gamma\sb{k} D\sb{k} + \theta\sb{0} \text{isWeekend} + \sum\sb{k=1}\sp{2} \theta\sb{k} (D\sb{k} \times \text{isWeekend}) + \epsilon$$

여기서 $D\sb{k}$는 상권 유형을 나타내는 더미 변수(더미 기준 범주: University)이며, $D\sb{1}$은 Office 상권, $D\sb{2}$는 Leisure 상권을 지칭합니다. 

#### Nested ANOVA F-test
교호작용이 통계적으로 유의미하게 대여량을 설명하는지 증명하기 위해 교호작용 항($D\sb{k} \times \text{isWeekend}$)이 생략된 축소 모형(Reduced Model)과 전체 모형(Full Model) 간의 잔차 제곱합(Residual Sum of Squares, RSS)을 비교하는 F-검정을 가동합니다.

$$F = \frac{(SSE\sb{\text{Reduced}} - SSE\sb{\text{Full}}) / (df\sb{\text{Reduced}} - df\sb{\text{Full}})}{SSE\sb{\text{Full}} / df\sb{\text{Full}}}$$

이 F-statistic은 분자 자유도 $df\sb{\text{Reduced}} - df\sb{\text{Full}}$과 분모 자유도 $df\sb{\text{Full}}$의 F-분포를 따릅니다. 검정 결과, 이 상호작용 효과를 포함했을 때의 다중선형회귀 적합도 분포와 분산 분석 결과는 통계적으로 모델의 설명력 상승에 기여함을 보였습니다.
...
여기서 대각 원소 $h\sb{ii}$는 $i$번째 관측치가 모델 예측값 결정에 미치는 수학적 영향력인 Leverage(지레대 값)를 나타냅니다.

#### Cook's Distance
레버리지와 이상치(Outlier)의 잔차 크기를 종합 고려해 추정 계수의 왜곡도를 판정하는 척도입니다.

$$D\sb{i} = \frac{\sum\sb{j\sb{1}}\sp{n} (\hat{Y}\sb{j} - \hat{Y}\sb{j(i)})\sp{2}}{p \cdot s\sp{2}} = \frac{e\sb{i}\sp{2}}{p \cdot s\sp{2}} \left( \frac{h\sb{ii}}{(1 - h\sb{ii})\sp{2}} \right)$$

여기서 $p$는 파라미터의 개수, $s\sp{2}$은 평균 잔차 제곱합(MSE)이며, $e\sb{i}$는 잔차입니다. Cook's Distance가 0.5 또는 1.0 이상인 경우 영향력이 비정상적으로 큰 이상치로 분류됩니다.
...
$$\ln\left(\frac{p}{1-p}\right) = \beta\sb{0} + \beta\sb{1} X\sb{1} + \beta\sb{2} X\sb{2} + \dots + \beta\sb{k} X\sb{k} \implies p = \frac{1}{1 + e\sp{-(\beta\sb{0} + \sum \beta\sb{j} X\sb{j})}}$$

특정 설명변수 $X\sb{j}$가 1단위 증가할 때 Target이 1(High Demand)이 될 오즈비(Odds Ratio)는 $e\sp{\beta\sb{j}}$로 계산됩니다.

#### LDA (선형판별분석) vs QDA (이차판별분석)
* **LDA:** 모든 범주가 공통의 공분산 행렬 $\Sigma$를 공유한다고 가정하므로 범주 간 결정 경계가 선형 형태를 띱니다. 범주 $k$에 대한 판별 함수 $\delta\sb{k}(x)$는 다음과 같습니다.
  $$\delta\sb{k}(x) = x^{T} \Sigma^{-1} \mu\sb{k} - \frac{1}{2} \mu\sb{k}^{T} \Sigma^{-1} \mu\sb{k} + \ln \pi\sb{k}$$
* **QDA:** 각 범주가 고유한 공분산 행렬 $\Sigma\sb{k}$를 갖는다고 가정하여 판별 경계가 이차(Quadratic) 곡선 형태를 가집니다.
  $$\delta\sb{k}(x) = -\frac{1}{2} \ln |\Sigma\sb{k}| - \frac{1}{2}(x - \mu\sb{k})^{T} \Sigma\sb{k}^{-1}(x - \mu\sb{k}) + \ln \pi\sb{k}$$

#### KNN (K-최근접 이웃)
새로운 관측치 $x\sb{0}$ 주변의 가장 가까운 이웃 데이터 포인트들의 모임인 $\mathcal{N}\sb{0}$을 탐색하고, 다수결 방식으로 범주 확률을 부여하는 대표적 비모수 모델입니다.

$$P(Y = j \mid X = x\sb{0}) = \frac{1}{K} \sum\sb{i \in \mathcal{N}\sb{0}} I(y\sb{i} = j)$$
...
데이터 포인트 $(X\sb{i}, Y\sb{i})$ 자체를 행 단위로 복원추출하여 가상 표본 $B$개를 구축합니다. 개별 가상 데이터셋을 통해 추정된 계수를 $\hat{\beta}\sp{*}\sb{b}$라 할 때, 최종적인 부트스트랩 표준오차 $\text{SE}\sb{\text{boot}}(\hat{\beta})$는 다음과 같습니다.

$$\text{SE}\sb{\text{boot}}(\hat{\beta}) = \sqrt{\frac{1}{B-1} \sum\sb{b=1}\sp{B} (\hat{\beta}\sp{*}\sb{b} - \bar{\beta}\sp{*})\sp{2}}$$
...
$$\min\sb{\beta} \sum\sb{i\sb{1}}\sp{n} \left( y\sb{i} - \beta\sb{0} - \sum\sb{j\sb{1}}\sp{p} \beta\sb{j} x\sb{ij} \right)^{2} + \lambda \sum\sb{j\sb{1}}\sp{p} |\beta\sb{j}|$$
...
$$\min\sb{\beta} \sum\sb{i\sb{1}}\sp{n} \left( y\sb{i} - \beta\sb{0} - \sum\sb{j\sb{1}}\sp{p} \beta\sb{j} x\sb{ij} \right)^{2} + \lambda \sum\sb{j\sb{1}}\sp{p} \beta\sb{j}\sp{2}$$
...
* **PLS:** 차원 축소 유도 시 타겟 $Y$와의 공분산 구조를 동시에 반영할 수 있도록 선형 결합 가중치 $w$를 직접 구합니다.
  $$\max\sb{w} \text{Cov}^{2}(Xw, Y) = \max\sb{w} \text{Var}(Xw) \cdot \text{Corr}^{2}(Xw, Y) \quad \text{s.t.} \quad \|w\|_{2}^{2} = 1$$
...
* **Smoothing Spline:** 모든 데이터 포인트에서 매끄럽게 연결되는 다항식 곡선을 도출하는 방법론으로, 오차 제곱합과 곡선의 거친 정도를 나타내는 2차 미분 적분 값을 절충합니다.
  $$\min\sb{g} \sum\sb{i\sb{1}}\sp{n} (y\sb{i} - g(x\sb{i}))\sp{2} + \lambda \int (g''(t))^{2} dt$$
...
특정 관측 거점 $x\sb{0}$를 기준으로 윈도우(Span) 이웃 영역을 정하고, 중심점과의 거리 차이를 Tricube 가중치 커널을 사용해 계산하여 국소적 가중 선형 적합을 수행합니다.

$$w\sb{i}(x) = \left( 1 - \left| \frac{x\sb{i} - x}{d(x)} \right|^3 \right)^3 I\left( \left| \frac{x\sb{i} - x}{d(x)} \right| < 1 \right)$$
...
$$\text{rentals} = \beta\sb{0} + f\sb{1}(\text{temp}) + f\sb{2}(\text{pm10}) + f\sb{3}(\text{precip}) + f\sb{4}(\text{stationType}) + \dots + \epsilon$$
...
#### Durbin-Watson 검정
시계열적 특성을 가진 잔차 벡터 $e\sb{t}$에 순차적 1차 자기상관($AR(1)$) 관계가 잔존하는지 판정합니다.

$$d = \frac{\sum\sb{t\sb{2}}\sp{T} (e\sb{t} - e\sb{t\sb{-1}})\sp{2}}{\sum\sb{t\sb{1}}\sp{T} e\sb{t}\sp{2}}$$
...
$$\text{isCircular}\sb{i} = \begin{cases} 1 & \text{if } \text{originStation}\sb{i} = \text{destinationStation}\sb{i} \\ 0 & \text{if } \text{originStation}\sb{i} \neq \text{destinationStation}\sb{i} \end{cases}$$
